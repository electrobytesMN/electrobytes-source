...
*Setup instructions for a Gitian build of electrobytes Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the electrobytes
...
- [Building electrobytes Core](#building-electrobytes-core)
...
Clone the git repositories for electrobytes Core and Gitian.
...
in the electrobytes Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building electrobytes Core
----------------

To build electrobytes Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the electrobytes Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/electrobytes/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/electrobytespay/electrobytes
...
For example:
```bash
URL=https://github.com/crowning-/electrobytes.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit electrobytes=${COMMIT} --url electrobytes=${URL} ../electrobytes/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit electrobytes=${COMMIT} --url electrobytes=${URL} ../electrobytes/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit electrobytes=${COMMIT} --url electrobytes=${URL} ../electrobytes/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the electrobytes git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../electrobytes/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/electrobytes
SIGPATH=/some/root/path/electrobytes-detached-sigs

./bin/gbuild --url electrobytes=${BTCPATH},signature=${SIGPATH} ../electrobytes/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/electrobytes-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/electrobytes-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/electrobytes-osx-build.assert
```
...
[electrobytes/gitian.sigs](https://github.com/electrobytespay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
