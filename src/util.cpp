...
const char * const BITCOIN_CONF_FILENAME = "electrobytes.conf";
const char * const BITCOIN_PID_FILENAME = "electrobytesd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "electrobytes" is a composite category enabling all electrobytes-related debug output
                if(ptrCategory->count(std::string("electrobytes"))) {
...
#else
    const char* pszModule = "electrobytes";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\electrobytes
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\electrobytes
    // Mac: ~/Library/Application Support/electrobytes
    // Unix: ~/.electrobytes
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "electrobytes";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/electrobytes";
#else
    // Unix
    return pathRet / ".electrobytes";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty electrobytes.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override electrobytes.conf
