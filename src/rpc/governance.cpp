...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in electrobytes.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("electrobytes.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "electrobytes",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "electrobytes",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "electrobytes",               "gobject",                &gobject,                true,  {} },
    { "electrobytes",               "voteraw",                &voteraw,                true,  {} },
